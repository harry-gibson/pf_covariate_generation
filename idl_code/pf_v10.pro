pro Pf_v10
  ...
  ;Malaria variable #9
  ;This is a per-month & year (144 layer) product 
  ;One input is annual (12-year), one is monthly synoptic
  ...
  input_path_1 = 'F:\5km_mastergrid_aligned\LST_day\mean\'
  ...
  ;input_file_2 = 'F:\5km_mastergrid_aligned\Poverty\srtm_1km_5km.max.tif'
  
  ; Define output information
  output_path = 'F:\5km_mastergrid_aligned\Pf_cubes\V10\'
  output_prefix ='Pf.V10.'
  ...
  input_filenamelist_1=FINDFILE(input_path_1 + '*' + input_suffix_1)
  input_filenamelistsize_1=size(input_filenamelist_1)
  input_nfilename_1 = input_filenamelistsize_1[1]  
  template_filename_1 = input_filenamelist_1[0]

  input_filenamelist_2=FINDFILE(input_path_2 + '*' + input_suffix_2)
  input_filenamelistsize_2=size(input_filenamelist_2)
  input_nfilename_2 = input_filenamelistsize_2[1]
  template_filename_2 = input_filenamelist_2[0]
  ...
  For h = 0, input_nfilename_1 - 1 Do Begin ; minus 2 to skip the annual synoptic .tif
    input_filename_1 = input_filenamelist_1[h]
    
    ; Split up the file name to remove the path of the EVI file
    str1 = STRSPLIT(input_filename_1, '\', /EXTRACT)
    n1   = SIZE(str1)
    n2   = n1[1] - 1   
    ...
    ; Handle the 1 month lag
    month_long = long(month)
    month_output_long = month_long + var_1_lag
    If month_output_long EQ 13 Then month_output_long = 1
    ...
    ENVI_OPEN_FILE, input_filename_1, r_fid = fid_input_1
    var_1_raster = ENVI_GET_DATA(fid = fid_input_1, dims = dims_template, pos=0)
    ENVI_FILE_MNG, ID = fid_input_1, /REMOVE
    PRINT, 'Processing input file ', input_filename_1
    
    For g = 0, input_nfilename_2 - 1 Do Begin ; minus 2 to skip the annual synoptic .tif
      input_filename_2 = input_filenamelist_2[g]
      
      ; Split up the file name to remove the path of the EVI file
      str1 = STRSPLIT(input_filename_2, '\', /EXTRACT)
      n1   = SIZE(str1)
      n2   = n1[1] - 1
      str2 = str1[n2] ; get the acual EVI file name from the full path
      str3 = STRSPLIT(str2, '.', /EXTRACT)
      ...
      ENVI_OPEN_FILE, input_filename_2, r_fid = fid_input_2
      var_2_raster = ENVI_GET_DATA(fid = fid_input_2, dims = dims_template, pos=0)
      ENVI_FILE_MNG, ID = fid_input_2, /REMOVE
      PRINT, 'Processing input file ', input_filename_2
      ...
      ;output_filename = output_path + output_prefix + '0.' + month + '.tif'
      ;output_filename = output_path + output_prefix + year + '.0.tif'
      ;output_filename = output_path + output_prefix + year + '.' + month + '.tif'
      output_filename = output_path + output_prefix + year + '.' + month_output + '.tif'
