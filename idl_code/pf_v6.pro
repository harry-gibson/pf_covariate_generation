pro Pf_v6
  ...
  ;Malaria variable #6
  ; Var 1 monthly, Var 2 monthly synoptic
  ...
  input_path_1 = 'F:\5km_mastergrid_aligned\EVI\max\
  input_suffix_1 = '.max.tif'
  ...
  output_path = 'F:\5km_mastergrid_aligned\Pf_cubes\V6\'
  output_prefix ='Pf.V6.'
  ...
  var_1_adjustment_factor = 0
  var_2_adjustment_factor = 0
  
  var_1_lag = 4
  var_2_lag = 4
  
  ;ratio_threshold = 0.55 ; The per-variable (i.e., one half of the interaction) correction factor for ratios to avoid extreme output (based on the min value of the training set)
  ...
  For h = 167, input_nfilename - 1 Do Begin
    ...
    ; In this case we're iterating on the second variable, which has a lag of -4
    ...
    ; Meanwhile the synoptic variable has a lag of 0, which (relative to the value of the file name) is 4 months ahead
    ; for example, if I were opening file named 2007.03, it will produce results for 2007.07, and require the synoptic value for July (07)
    month_long = long(month)
    ...
    For a = 0, ns_template -1 Do Begin
      For b = 0, nl_template -1 Do Begin
        If template_raster[a,b] NE no_data_value AND var_1_raster[a,b] NE no_data_value AND var_2_raster[a,b] NE no_data_value AND var_2_raster[a,b] NE 0.0 Then Begin
          v1 = (var_1_raster[a,b] + var_1_adjustment_factor)^(1.0/3.0)
