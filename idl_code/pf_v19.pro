pro Pf_v19
  ...
  ;Malaria variable #19
  ;Dynamic V1 adjusted with a synoptic V1 alt, and then a synoptic V2
  ...
  input_path_1 = 'F:\5km_mastergrid_aligned\EVI\min\'
  input_suffix_1 = '.min.tif'
  ;input_file_1 = 'F:\5km_mastergrid_aligned\LST_night\mean\synoptic\annual.synoptic.mean.tif'
  
  input_path_1_alt = 'F:\5km_mastergrid_aligned\EVI\min\'
  input_suffix_1_alt = '.synoptic.mean.tif'
  ;input_file_1_alt = 'F:\5km_mastergrid_aligned\LST_night\mean\synoptic\annual.synoptic.mean.tif'
  
  input_path_2 = 'F:\5km_mastergrid_aligned\LST_day\stdev\'
  input_suffix_2 = '.synoptic.mean.tif'
  ;input_file_2 = 'F:\5km_mastergrid_aligned\worldclim\prec57a0.tif'
  
  ; Define output information
  output_path = 'F:\5km_mastergrid_aligned\Pf_cubes\V19\'
  output_prefix ='Pf.V19.'
  ...
  var_1_adjustment_factor = 1.16171
  var_2_adjustment_factor = 1.0
  
  ;var_1_lag = 4 
  var_2_lag = 2
  
  ;ratio_threshold = 1.0 ; The per-variable (i.e., one half of the interaction) correction factor for ratios to avoid extreme output (based on the min value of the training set)
  ...
  For h = 167, input_nfilename - 1 Do Begin 
    ...
    ; Meanwhile the var 2 a lag of -3, which (relative to the value of the file name) is 1 months ahead    
    ; for example, if I were opening file named 2007.03, it will produce results for 2007.07, and require the var_2 data for 2007.04
    month_long = long(month)
    month_var_2_long = month_long - var_2_lag
    If month_var_2_long LE 0 Then Begin
      month_var_2_long = month_var_2_long + 12
    ;  year_var_2_long = long(year) + 1
    EndIf Else year_var_2_long = long(year)
    If month_var_2_long LT 10 Then month_var_2 = '0' + strtrim(string(month_var_2_long),1) Else month_var_2 = strtrim(string(month_var_2_long),1)
    ;year_var_2 =  strtrim(string(year_var_2_long),1)

    ;Construct the var_1_alt name
    input_fileame_var_1_alt = input_path_1_alt + month + input_suffix_1_alt

    ;Construct the var_2 name 
    ;If long(year) EQ 2000 Then input_filename_var_2 = 'F:\5km_mastergrid_aligned\IGBP_landcover\2001.fraction.class.14.tif' Else input_filename_var_2 = input_path_2 + year + input_suffix_2 
    input_filename_var_2 = input_path_2 + month_var_2 + input_suffix_2

    ENVI_OPEN_FILE, input_filename_var_2, r_fid = fid_input_var_2
    var_2_raster = ENVI_GET_DATA(fid = fid_input_var_2, dims = dims_template, pos=0)
    ENVI_FILE_MNG, ID = fid_input_var_2, /REMOVE
    PRINT, 'Processing input file ', input_filename_var_2
    ...
    ENVI_OPEN_FILE, input_fileame_var_1_alt , r_fid = fid_input
    var_1_alt_raster = ENVI_GET_DATA(fid = fid_input, dims = dims_template, pos=0)
    ENVI_FILE_MNG, ID = fid_input, /REMOVE
    PRINT, 'Processing input file ', input_fileame_var_1_alt 
    ...
    ; t7 alt            :   ((var_1_raster[a,b] - var_1_raster_alt[a,b]) + var_1_adjustment_factor)^2
    ...
    For a = 0, ns_template -1 Do Begin
      For b = 0, nl_template -1 Do Begin
        If template_raster[a,b] NE no_data_value AND var_1_raster[a,b] NE no_data_value AND var_2_raster[a,b] NE no_data_value and var_1_alt_raster[a,b] NE no_data_value Then Begin
          v1 =  ((var_1_raster[a,b] - var_1_alt_raster[a,b]) + var_1_adjustment_factor)
          If v1 GT 1.0 Then v1 = alog10(v1) Else v1 = 0
          v2 = (var_2_raster[a,b] + var_2_adjustment_factor)^var_2_BoxCox
          output_raster[a,b] = v1 * v2
          ;If a EQ 1262 AND b EQ 929 Then Begin
          ;  t1 = var_1_raster[a,b]
          ;  t1a = var_1_alt_raster[a,b]
          ;  t1b = t1 - t1a
